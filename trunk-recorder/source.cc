  ...
  for (std::vector<Detected_Signal>::iterator it = signals.begin(); it != signals.end(); it++) {
    ...
    for (std::vector<Recorder *>::iterator it = recorders.begin(); it != recorders.end(); it++) {
      ...
      if (!recorder->is_enabled()) {
        ...
        BOOST_LOG_TRIVIAL(info) << "\t[ " << recorder->get_num() << " ] " << recorder->get_type_string() << "\tEnabled - Freq: " << format_freq(recorder->get_freq()) << recorder->get_squelch_type_string() << "\t Detected Signal: " << floor(rssi) << "dBM (Threshold: " << floor(threshold) << "dBM)";
      }
    }
  }
}
